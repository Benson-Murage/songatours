...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Authenticate user via getUser (not getClaims which doesn't exist)
    ...
    const subtotal = effectivePrice * guestsNum;
    ...
    // Deposit percentage is informational only — shown to user, NOT recorded as paid
    const depositPct = Number(tour.deposit_percentage || 30);
    const suggestedDeposit = Math.round(totalPrice * (depositPct / 100));
    ...
    // --- Insert booking (booking_reference generated by DB trigger) ---
    // CRITICAL: amount_paid MUST start at 0. Payments only come from admin actions.
    // The DB trigger (sync_booking_payment_fields) will derive balance_due,
    // payment_status, deposit_amount, overpayment_amount from amount_paid + total_price.
    ...
    if (referral_code) {
      ...
      if (referral && referral.referrer_id !== userId) {
        await supabaseAdmin.from("referrals").insert({
          ...
          reward_amount: Math.round(totalPrice * 0.05),
          status: "completed",
        });
      }
    }
